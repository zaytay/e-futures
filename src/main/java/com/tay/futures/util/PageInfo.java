...
/**
 * Created by zaytay on 2015/10/26.
 */
public class PageInfo<T> implements Iterable<T> {
    // -- 公共变量 --//
    ...
    /** 默认为id倒序的构造方法 */
    public PageInfo(int pageNo, int pageSize, Map<String, Object> param) {
        setPageNo(pageNo);
        setPageSize(pageSize);
        setParam(param);
        setOrderBy("id");
     ...
     *            正序还是倒序
     */
    public PageInfo(int pageNo, int pageSize, Map<String, Object> param, String orderBy, String order) {
        setPageNo(pageNo);
        setPageSize(pageSize);
        setParam(param);
        setOrderBy(orderBy);
    ...
    public List<Long> getSlider() {
        ...
        long startPageNumber = Math.max(pageNo - halfSize + 1, 1);
        ...
        if (endPageNumber - startPageNumber >= count) {
            startPageNumber = Math.max(endPageNumber - count - 1, 1);
        }
